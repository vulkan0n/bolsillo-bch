  ...
  reducers: {
    ...
    createDefaultWallet(state, action) {
      if (state.wallets.length === 0) {
        ...
        let isUnusedName = false;
        let autoGeneratedWalletName = `Autogenerated Wallet (${
          state.wallets.length + 1
        })`;
        while (isUnusedName === false) {
          const existingWalletNames = state.wallets.map(({ name }) => name);
          const isMatch = existingWalletNames?.includes(
            autoGeneratedWalletName
          );
          if (isMatch) {
            // 0 to 100 000
            const randomNumber = Math.floor(Math.random() * 100000);
            autoGeneratedWalletName = `Autogenerated Wallet (${randomNumber})`;
          } else {
            isUnusedName = true;
          }
        }

        const autogeneratedWallet = {
          name: autoGeneratedWalletName,
          description: "A Bitcoin Cash wallet",
          mnemonic: action.payload.mnemonic,
          derivationPath: action.payload.derivationPath,
          cashaddr: action.payload.cashaddr,
        ...
        state.wallets = [...state.wallets, autogeneratedWallet];
        state.activeWalletName = autoGeneratedWalletName;
      }
    },
