...
There is also now an export script so you can give a .txt file to somebody willing to review and update translations in a specific language `node automation/exportTranslations.js es`, which can be reimported using the import script (provide pathname to the updated file). Note that afterwards ASCII characters need to be specifically checked and find/replaced e.g. `&#39;` needs to be replaced with apostrophe `'`.
