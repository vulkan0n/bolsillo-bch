...
export default function NftDetailModal({
  ...
  return (
    <Overlay>
      ...
      <button
        type="button"
        className="absolute top-3 right-3 z-20 w-8 h-8 flex items-center justify-center rounded-full bg-neutral-900/50 text-white cursor-pointer"
        onClick={onClose}
      >
        <CloseOutlined />
      </button>
      ...
      >
        ...
        <div className="flex-1 flex items-center justify-center p-2 min-h-0 pointer-events-auto">
          {hasPrev && (
            <button
              type="button"
              className="absolute left-2 z-20 w-8 h-8 flex items-center justify-center rounded-full bg-neutral-900/30 text-white/60 cursor-pointer"
              onPointerUp={(e) => {
                e.stopPropagation();
          ...
          {imageUri && (
            <img
              src={imageUri}
              className={`max-w-full max-h-full object-contain rounded-lg ${isImageLoaded ? "opacity-100" : "opacity-0 absolute"}`}
          ...
          {hasNext && (
            <button
              type="button"
              className="absolute right-2 z-20 w-8 h-8 flex items-center justify-center rounded-full bg-neutral-900/30 text-white/60 cursor-pointer"
              onPointerUp={(e) => {
                e.stopPropagation();
