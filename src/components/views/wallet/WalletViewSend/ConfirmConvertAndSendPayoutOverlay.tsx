      ...
      <div className="fixed top-0 w-full h-screen z-40 flex flex-col items-center justify-center bg-transparent">
        {(convertInfo.showQuoteChangedMessage ? (
          <div className="mx-2 my-2">
            { translate(translations.confirm_convert_and_send_payout_quote_changed_message) }
          </div>
          ) : false)}
        <div className="text-center">
          <div className="mx-2 my-2">
            { translate(translations.confirm_convert_and_send_payout_message_pre_token_amount) }
            <span
            ...
            <span className="">
              <NumberFormat number={convertInfo.spend.tokenAmount} scalar={-convertInfo.tokenEntity.token.decimals} decimals={convertInfo.tokenEntity.token.decimals} />
            </span>
            <span className="">
              { convertInfo.tokenEntity.token.symbol }
            </span>
            { translate(translations.confirm_convert_and_send_payout_message_post_token_amount) }
            <span>{txfeeText}</span>
            { translate(translations.confirm_convert_and_send_payout_message_post_txfee_amount) }
            <span>{changeText}</span>
            { translate(translations.confirm_convert_and_send_payout_message_post_change_amount) }
          </div>
        </div>
